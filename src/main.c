...
int load_groups(json_t* json, uint32_t* out)
{
    ...
    for (int i = 0; i < json_array_size(json); i++)
    {
        ...
        if (strcmp(json_string_value(group_name), "brakes") == 0)
            groups |= TRACK_GROUP_BRAKES;
        else if (strcmp(json_string_value(group_name), "turns") == 0)
            groups |= TRACK_GROUP_TURNS;
        else if (strcmp(json_string_value(group_name), "gentle_slopes") == 0)
            groups |= TRACK_GROUP_GENTLE_SLOPES;
        else if (strcmp(json_string_value(group_name), "steep_slopes") == 0)
            groups |= TRACK_GROUP_STEEP_SLOPES | TRACK_GROUP_GENTLE_SLOPES;
        else if (strcmp(json_string_value(group_name), "vertical_slopes") == 0)
            groups |= TRACK_GROUP_VERTICAL_SLOPES | TRACK_GROUP_STEEP_SLOPES | TRACK_GROUP_GENTLE_SLOPES;
        else if (strcmp(json_string_value(group_name), "diagonals") == 0)
            groups |= TRACK_GROUP_DIAGONALS;
        else if (strcmp(json_string_value(group_name), "sloped_turns") == 0)
            groups |= TRACK_GROUP_SLOPED_TURNS | TRACK_GROUP_GENTLE_SLOPES | TRACK_GROUP_TURNS;
        else if (strcmp(json_string_value(group_name), "banked_turns") == 0)
            groups |= TRACK_GROUP_BANKED_TURNS | TRACK_GROUP_TURNS;
        else if (strcmp(json_string_value(group_name), "banked_sloped_turns") == 0)
            groups |= TRACK_GROUP_BANKED_SLOPED_TURNS | TRACK_GROUP_GENTLE_SLOPES | TRACK_GROUP_TURNS
                | TRACK_GROUP_BANKED_TURNS;
        else if (strcmp(json_string_value(group_name), "s_bends") == 0)
            groups |= TRACK_GROUP_S_BENDS;
        else if (strcmp(json_string_value(group_name), "helices") == 0)
            groups |= TRACK_GROUP_HELICES | TRACK_GROUP_BANKED_TURNS | TRACK_GROUP_TURNS;
        else if (strcmp(json_string_value(group_name), "large_slope_transitions") == 0)
            groups |= TRACK_GROUP_LARGE_SLOPE_TRANSITIONS | TRACK_GROUP_STEEP_SLOPES | TRACK_GROUP_GENTLE_SLOPES;
        else if (strcmp(json_string_value(group_name), "barrel_rolls") == 0)
            groups |= TRACK_GROUP_BARREL_ROLLS;
        else if (strcmp(json_string_value(group_name), "quarter_loops") == 0)
            groups |= TRACK_GROUP_QUARTER_LOOPS | TRACK_GROUP_VERTICAL_SLOPES | TRACK_GROUP_STEEP_SLOPES
                | TRACK_GROUP_GENTLE_SLOPES;
        else if (strcmp(json_string_value(group_name), "corkscrews") == 0)
            groups |= TRACK_GROUP_CORKSCREWS;
        else if (strcmp(json_string_value(group_name), "half_loops") == 0)
            groups |= TRACK_GROUP_HALF_LOOPS;
        else if (strcmp(json_string_value(group_name), "boosters") == 0)
            groups |= TRACK_GROUP_BOOSTERS;
        else if (strcmp(json_string_value(group_name), "launched_lifts") == 0)
            groups |= TRACK_GROUP_LAUNCHED_LIFTS;
        else if (strcmp(json_string_value(group_name), "turn_bank_transitions") == 0)
            groups |= TRACK_GROUP_TURN_BANK_TRANSITIONS;
        else
        {
...
int load_track_type(track_type_t* track_type, json_t* json)
{
    ...
    if (flags != NULL)
    {
        ...
        for (int i = 0; i < json_array_size(flags); i++)
        {
            ...
            if (strcmp(json_string_value(flag_name), "has_lift") == 0)
                track_type->flags |= TRACK_HAS_LIFT;
            else if (strcmp(json_string_value(flag_name), "has_supports") == 0)
                track_type->flags |= TRACK_HAS_SUPPORTS;
            else if (strcmp(json_string_value(flag_name), "semi_split") == 0)
                track_type->flags |= TRACK_SEMI_SPLIT;
            else
            {
    ...
    if (track_type->flags & TRACK_HAS_LIFT)
    {
        ...
        if (groups != NULL)
        {
            ...
            if (load_groups(groups, &(track_type->lift_groups)))
                return 1;
        }
    }
    ...
    if (length != NULL && json_is_number(length))
        track_type->length = json_number_value(length) * TILE_SIZE;
    else
    {
    ...
    if (z_offset != NULL && json_is_number(z_offset))
        track_type->z_offset = json_number_value(z_offset);
    else
    {
    ...
    {
        if (json_is_number(support_spacing))
            track_type->support_spacing = json_number_value(support_spacing) * TILE_SIZE;
        else
        {
    ...
    for (int i = 0; i < NUM_MODELS; i++)
    {
        ...
        else if (result == 1)
        {
            ...
            for (int j = 0; j < i; j++)
                mesh_destroy(&(track_type->models[j]));
            return 1;
        }
    }
...
int main(int argc, char** argv)
{
    ...
    if (json_base_dir != NULL && json_is_string(json_base_dir))
        base_dir = json_string_value(json_base_dir);
    else
        printf("Error: No property \"base_directory\" found\n");

    const char* sprite_dir = NULL;
    json_t* json_sprite_dir = json_object_get(track, "sprite_directory");
    if (json_sprite_dir != NULL && json_is_string(json_sprite_dir))
        sprite_dir = json_string_value(json_sprite_dir);
    else
        printf("Error: No property \"sprite_directory\" found\n");

    const char* spritefile_in = NULL;
    json_t* json_spritefile_in = json_object_get(track, "spritefile_in");
    if (json_spritefile_in != NULL && json_is_string(json_spritefile_in))
        spritefile_in = json_string_value(json_spritefile_in);
    else
        printf("Error: No property \"spritefile_in\" found\n");

    const char* spritefile_out = NULL;
    json_t* json_spritefile_out = json_object_get(track, "spritefile_out");
    if (json_spritefile_out != NULL && json_is_string(json_spritefile_out))
        spritefile_out = json_string_value(json_spritefile_out);
    else
        printf("Error: No property \"spritefile_out\" found\n");
