...
track_point_t get_track_point(track_point_t (*curve)(float distance), int flags, float z_offset, float length, float u)
{
    ...
    if (flags & TRACK_DIAGONAL)
        track_point.position.x += 0.5 * TILE_SIZE;
    track_point.position.y += z_offset - 2 * CLEARANCE_HEIGHT;
    if (!(flags & TRACK_VERTICAL))
        track_point.position.z -= 0.5 * TILE_SIZE;
    return track_point;
}
...
vertex_t track_transform(vector3_t vertex, vector3_t normal, void* data)
{
    ...
    vertex_t out;
    out.vertex = change_coordinates(vector3_add(
        track_point.position,
        vector3_add(vector3_mult(track_point.normal, vertex.y), vector3_mult(track_point.binormal, vertex.x))));
    out.normal = change_coordinates(vector3_add(
        vector3_mult(track_point.tangent, normal.z),
        vector3_add(vector3_mult(track_point.normal, normal.y), vector3_mult(track_point.binormal, normal.x))));
    return out;
}
...
vertex_t base_transform(vector3_t vertex, vector3_t normal, void* data)
{
    ...
    out.vertex = change_coordinates(vector3_add(
        track_point.position,
        vector3_add(vector3_mult(vector3(0, 1, 0), vertex.y), vector3_mult(track_point.binormal, vertex.x))));
    out.normal = change_coordinates(vector3_add(
        vector3_mult(track_point.tangent, normal.z),
        vector3_add(vector3_mult(track_point.normal, normal.y), vector3_mult(track_point.binormal, normal.x))));
    return out;
...
void render_track_section(
    context_t* context, track_section_t* track_section, track_type_t* track_type, int extrude_behind, int track_mask,
    int rendered_views, image_t* images, int subtype)
{
    ...
    if (extrude_behind)
        num_meshes++; // TODO make extruded section straight
    ...
    for (int i = 0; i < num_meshes; i++)
    {
        ...
        if ((track_type->models_loaded & (1 << MODEL_BASE)) && (track_type->flags & TRACK_HAS_SUPPORTS)
            && !(track_section->flags & TRACK_NO_SUPPORTS))
    ...
    if (track_section->flags & TRACK_SPECIAL_MASK)
    {
        ...
        if (track_type->models_loaded & (1 << index))
        {
            ...
            if ((track_section->flags & TRACK_SPECIAL_MASK) != TRACK_SPECIAL_VERTICAL_TWIST_RIGHT
                && (track_section->flags & TRACK_SPECIAL_MASK) != TRACK_SPECIAL_BARREL_ROLL_RIGHT
                && (track_section->flags & TRACK_SPECIAL_MASK) != TRACK_SPECIAL_CORKSCREW_RIGHT)
            {
                mat.entries[6] *= -1;
                mat.entries[7] *= -1;
                mat.entries[8] *= -1;
            ...
            context_add_model(
                context, &(track_type->models[index]),
                transform(
                    mat,
                    vector3(
                        !(track_section->flags & TRACK_VERTICAL) ? -0.5 * TILE_SIZE : 0, z_offset - 2 * CLEARANCE_HEIGHT, 0)),
                track_mask);
        }
    }
    ...
    if ((track_type->flags & TRACK_HAS_SUPPORTS) && !(track_section->flags & TRACK_NO_SUPPORTS))
    {
        ...
        int entry = 0;
        int exit = 0;
        if (track_section->flags & TRACK_ENTRY_BANK_LEFT)
            entry = DENOM;
        else if (track_section->flags & TRACK_ENTRY_BANK_RIGHT)
            entry = -DENOM;
        else
            entry = 0;

        if (track_section->flags & TRACK_EXIT_BANK_LEFT)
            exit = DENOM;
        else if (track_section->flags & TRACK_EXIT_BANK_RIGHT)
            exit = -DENOM;
        else
            exit = 0;
        ...
        for (int i = 0; i < num_supports + 1; i++)
        {
            ...
            track_point_t track_point = get_track_point(
                track_section->curve, track_section->flags, z_offset, track_section->length, i * support_step);

            track_point_t support_point = only_yaw(track_point);

            matrix_t rotation = matrix(
                support_point.binormal.x, support_point.normal.x, support_point.tangent.x, support_point.binormal.y,
                support_point.normal.y, support_point.tangent.y, support_point.binormal.z, support_point.normal.z,
                support_point.tangent.z);
            if (bank_angle < 0)
                rotation = matrix_mult(views[2], rotation);

            vector3_t translation = change_coordinates(support_point.position);
            translation.y -= pivot
                    / sqrt(track_point.tangent.x * track_point.tangent.x + track_point.tangent.z * track_point.tangent.z)
                - pivot;

            context_add_model(
                context, &(track_type->models[get_support_index(bank_angle)]), transform(rotation, translation), track_mask);
        }
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        {
            if (track_mask)
                context_render_silhouette(context, rotate_y(0.5 * i * M_PI), images + i);
            else
                context_render_view(context, rotate_y(0.5 * i * M_PI), images + i);
        }
    }
...
int is_in_mask(int x, int y, mask_t* mask)
{
    ...
    {
        if (x >= mask->rects[i].x_lower && x < mask->rects[i].x_upper && y >= mask->rects[i].y_lower
            && y < mask->rects[i].y_upper)
            return 1;
    }
    return 0;
}

void write_track_section(
    context_t* context, track_section_t* track_section, track_type_t* track_type, const char* base_directory,
    const char* filename, json_t* sprites, int subtype, image_t* overlay)
{
    ...
    if (track_section->flags & TRACK_EXTRUDE_BEHIND)
    {
        ...
        render_track_section(context, track_section, track_type, 0, 0, 0xF, full_sprites, subtype);

    if (overlay != NULL)
    {
        for (int i = 0; i < 4; i++)
            image_blit(full_sprites + i, overlay + i, 0, 12 - z_offset);
    }

    image_t track_masks[4];
    int track_mask_views = 0;
    for (int i = 0; i < 4; i++)
        track_mask_views |= (track_section->views[i].flags & VIEW_NEEDS_TRACK_MASK ? 1 : 0) << i;
    if (track_mask_views != 0)
        render_track_section(context, track_section, track_type, 0, 1, track_mask_views, track_masks, subtype);

    for (int angle = 0; angle < 4; angle++)
    {
        if (track_section->views[angle].num_sprites == 0)
            continue;
        ...
        for (int sprite = 0; sprite < view->num_sprites; sprite++)
        {
            ...
            if (view->num_sprites == 1)
                ...
                snprintf(relative_filename, 512, "%s_%d_%d.png", filename, angle + 1, sprite + 1);
            ...
            if (view->masks != NULL)
            {
                for (int x = 0; x < full_sprites[angle].width; x++)
                    ...
                    {
                        int in_mask = is_in_mask(
                            x + full_sprites[angle].x_offset,
                            y + full_sprites[angle].y_offset
                                + ((track_section->flags & TRACK_OFFSET_SPRITE_MASK) ? (z_offset - 8) : 0),
                            view->masks + sprite);
                        ...
                        if (view->masks[sprite].track_mask_op != TRACK_MASK_NONE)
                        {
                            ...
                            int in_track_mask = mask_x >= 0 && mask_y >= 0 && mask_x < track_masks[angle].width
                            ...
                            if (sprite < view->num_sprites - 1 && (view->masks[sprite].track_mask_op & TRACK_MASK_TRANSFER_NEXT)
                                && in_track_mask
                                && is_in_mask(
                                    x + full_sprites[angle].x_offset,
                                    y + full_sprites[angle].y_offset
                            ...
                            for (int i = 0; i < sprite; i++)
                            {
                                if (is_in_mask(
                                        ...
                                        y + full_sprites[angle].y_offset
                                            + ((track_section->flags & TRACK_OFFSET_SPRITE_MASK) ? (z_offset - 8) : 0),
                                        view->masks + i))
                                    in_mask = 0; // Note z offset untested
                            }
                        }

                        if (in_mask)
                        {
                            part_sprite.pixels[x + part_sprite.width * y] = full_sprites[angle]
                                                                                .pixels[x + full_sprites[angle].width * y];
                        }
                        else
                        {
        ...
        if (view->flags & VIEW_NEEDS_TRACK_MASK)
            image_destroy(track_masks + angle);
        image_destroy(full_sprites + angle);
    }
}

int write_track_subtype(
    context_t* context, track_type_t* track_type, json_t* sprites, const char* base_dir, const char* output_dir, int subtype)
{
    ...
    if (subtype == TRACK_SUBTYPE_LIFT)
        write_track_section(context, &flat_asymmetric, track_type, base_dir, output_path, sprites, subtype, flat_chain);
    else
    ...
    if (groups & TRACK_GROUP_GENTLE_SLOPES)
    {
        ...
        write_track_section(
            context, &flat_to_gentle_up, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? flat_to_gentle_up_chain : NULL);
        sprintf(output_path, "%.255sgentle_up_to_flat%s", output_dir, suffix);
        write_track_section(
            context, &gentle_up_to_flat, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_up_to_flat_chain : NULL);
        sprintf(output_path, "%.255sgentle%s", output_dir, suffix);
        write_track_section(
            context, &gentle, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_chain : NULL);
    }
    if (groups & TRACK_GROUP_STEEP_SLOPES)
    {
        sprintf(output_path, "%.255sgentle_to_steep_up%s", output_dir, suffix);
        write_track_section(
            context, &gentle_to_steep_up, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_to_steep_up_chain : NULL);
        sprintf(output_path, "%.255ssteep_to_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, &steep_to_gentle_up, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? steep_to_gentle_up_chain : NULL);
        sprintf(output_path, "%.255ssteep%s", output_dir, suffix);
        write_track_section(
            context, &steep, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? steep_chain : NULL);
    }
    ...
    if (groups & TRACK_GROUP_DIAGONALS)
    {
        ...
        write_track_section(
            context, &flat_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? flat_diag_chain : NULL);
        sprintf(output_path, "%.255sflat_to_gentle_up_diag%s", output_dir, suffix);
        write_track_section(
            context, &flat_to_gentle_up_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? flat_to_gentle_up_diag_chain : NULL);
        sprintf(output_path, "%.255sgentle_to_flat_up_diag%s", output_dir, suffix);
        write_track_section(
            context, &gentle_to_flat_up_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_to_flat_up_diag_chain : NULL);
        sprintf(output_path, "%.255sgentle_diag%s", output_dir, suffix);
        write_track_section(
            context, &gentle_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_diag_chain : NULL);
        sprintf(output_path, "%.255sgentle_to_steep_up_diag%s", output_dir, suffix);
        write_track_section(
            context, &gentle_to_steep_up_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? gentle_to_steep_up_diag_chain : NULL);
        sprintf(output_path, "%.255ssteep_to_gentle_up_diag%s", output_dir, suffix);
        write_track_section(
            context, &steep_to_gentle_up_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? steep_to_gentle_up_diag_chain : NULL);
        sprintf(output_path, "%.255ssteep_diag%s", output_dir, suffix);
        write_track_section(
            context, &steep_diag, track_type, base_dir, output_path, sprites, subtype,
            subtype == TRACK_SUBTYPE_LIFT ? steep_diag_chain : NULL);
    }
    ...
    if (groups & TRACK_GROUP_BANKED_TURNS)
    {
        ...
        write_track_section(
            context, semi_split ? &semi_split_left_bank : &left_bank, track_type, base_dir, output_path, sprites, subtype,
            NULL);
        ...
        if (groups & TRACK_GROUP_DIAGONALS)
        {
            ...
            write_track_section(
                context, &left_bank_to_gentle_up_diag, track_type, base_dir, output_path, sprites, subtype, NULL);
            sprintf(output_path, "%.255sright_bank_to_gentle_up_diag%s", output_dir, suffix);
            write_track_section(
                context, &right_bank_to_gentle_up_diag, track_type, base_dir, output_path, sprites, subtype, NULL);
            sprintf(output_path, "%.255sgentle_up_to_left_bank_diag%s", output_dir, suffix);
            write_track_section(
                context, &gentle_up_to_left_bank_diag, track_type, base_dir, output_path, sprites, subtype, NULL);
            sprintf(output_path, "%.255sgentle_up_to_right_bank_diag%s", output_dir, suffix);
            write_track_section(
                context, &gentle_up_to_right_bank_diag, track_type, base_dir, output_path, sprites, subtype, NULL);
            sprintf(output_path, "%.255sleft_bank_diag%s", output_dir, suffix);
            write_track_section(
                context, semi_split ? &semi_split_left_bank_diag : &left_bank_diag, track_type, base_dir, output_path, sprites,
                subtype, NULL);
        }

        sprintf(output_path, "%.255ssmall_turn_left_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_small_turn_left_bank : &small_turn_left_bank, track_type, base_dir, output_path,
            sprites, subtype, NULL);
        sprintf(output_path, "%.255smedium_turn_left_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_medium_turn_left_bank : &medium_turn_left_bank, track_type, base_dir, output_path,
            sprites, subtype, NULL);
        sprintf(output_path, "%.255slarge_turn_left_to_diag_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_large_turn_left_to_diag_bank : &large_turn_left_to_diag_bank, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255slarge_turn_right_to_diag_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_large_turn_right_to_diag_bank : &large_turn_right_to_diag_bank, track_type,
            base_dir, output_path, sprites, subtype, NULL);
    }
    ...
    if ((groups & TRACK_GROUP_SLOPED_TURNS) && (groups & TRACK_GROUP_STEEP_SLOPES))
    {
        ...
        write_track_section(
            context, &very_small_turn_right_steep_up, track_type, base_dir, output_path, sprites, subtype, NULL);
    }
    ...
    if (groups & TRACK_GROUP_BANKED_SLOPED_TURNS)
    {
        ...
        write_track_section(
            context, &gentle_up_to_gentle_up_left_bank, track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sgentle_up_to_gentle_up_right_bank%s", output_dir, suffix);
        write_track_section(
            context, &gentle_up_to_gentle_up_right_bank, track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sgentle_up_left_bank_to_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, &gentle_up_left_bank_to_gentle_up, track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sgentle_up_right_bank_to_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, &gentle_up_right_bank_to_gentle_up, track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sleft_bank_to_gentle_up_left_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_left_bank_to_gentle_up_left_bank : &left_bank_to_gentle_up_left_bank, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sright_bank_to_gentle_up_right_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_right_bank_to_gentle_up_right_bank : &right_bank_to_gentle_up_right_bank,
            track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sgentle_up_left_bank_to_left_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_gentle_up_left_bank_to_left_bank : &gentle_up_left_bank_to_left_bank, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255sgentle_up_right_bank_to_right_bank%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_gentle_up_right_bank_to_right_bank : &gentle_up_right_bank_to_right_bank,
            track_type, base_dir, output_path, sprites, subtype, NULL);
        ...
        write_track_section(
            context, semi_split ? &semi_split_small_turn_left_bank_gentle_up : &small_turn_left_bank_gentle_up, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255ssmall_turn_right_bank_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_small_turn_right_bank_gentle_up : &small_turn_right_bank_gentle_up, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255smedium_turn_left_bank_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_medium_turn_left_bank_gentle_up : &medium_turn_left_bank_gentle_up, track_type,
            base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255smedium_turn_right_bank_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_medium_turn_right_bank_gentle_up : &medium_turn_right_bank_gentle_up, track_type,
            base_dir, output_path, sprites, subtype, NULL);
    }
    ...
    if (groups & TRACK_GROUP_HELICES)
    {
        ...
        write_track_section(
            context, semi_split ? &semi_split_small_helix_left_up : &small_helix_left_up, track_type, base_dir, output_path,
            sprites, subtype, NULL);
        sprintf(output_path, "%.255ssmall_helix_right_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_small_helix_right_up : &small_helix_right_up, track_type, base_dir, output_path,
            sprites, subtype, NULL);
        sprintf(output_path, "%.255smedium_helix_left_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_medium_helix_left_up : &medium_helix_left_up, track_type, base_dir, output_path,
            sprites, subtype, NULL);
        sprintf(output_path, "%.255smedium_helix_right_up%s", output_dir, suffix);
        write_track_section(
            context, semi_split ? &semi_split_medium_helix_right_up : &medium_helix_right_up, track_type, base_dir, output_path,
            sprites, subtype, NULL);
    }
    ...
    if (groups & TRACK_GROUP_QUARTER_LOOPS)
    {
        ...
        write_track_section(
            context, semi_split ? &semi_split_quarter_loop_up : &quarter_loop_up, track_type, base_dir, output_path, sprites,
            subtype, NULL);
    }
    ...
    if (groups & TRACK_GROUP_TURN_BANK_TRANSITIONS)
    {
        ...
        write_track_section(
            context, &small_turn_left_bank_to_gentle_up, track_type, base_dir, output_path, sprites, subtype, NULL);
        sprintf(output_path, "%.255ssmall_turn_right_bank_to_gentle_up%s", output_dir, suffix);
        write_track_section(
            context, &small_turn_right_bank_to_gentle_up, track_type, base_dir, output_path, sprites, subtype, NULL);
    }
...
int write_track_type(
    context_t* context, track_type_t* track_type, json_t* sprites, const char* base_dir, const char* output_dir)
{
